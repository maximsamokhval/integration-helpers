...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//TODO (),20.08.19 , описать заполнение таблицы значений из параметров передачи 
...
Процедура ВыгрузитьТаблицуЗначенийСервер()
	...
	ПараметрыПередачи = ПоместитьОписаниеВСтруктуру();	
...
&НаСервере
Функция ПоместитьОписаниеВСтруктуру()
		
	ПредставлениеТЗ 	= СформироватьПредставлениеТаблицыЗначений(ПредставлениеТЗ); 	
	ПараметрыПередачи 	= Новый Структура;
	ПараметрыПередачи.Вставить("АдресХранилища", ПоместитьВоВременноеХранилище(ВнутрЗначениеОбъектаТЗ(),Новый УникальныйИдентификатор));
