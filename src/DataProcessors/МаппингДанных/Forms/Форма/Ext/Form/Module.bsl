...
&НаКлиенте
Процедура ОписаниеВнешнейТаблицы(Команда)
	
	ПараметрАдрес		= Новый Структура;
	ПараметрАдрес.Вставить("АдресХранилища", ПоместитьВоВременноеХранилище(Неопределено,УникальныйИдентификатор));
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияТаблицыЗначений",ЭтотОбъект,Новый Структура());
	ОткрытьФорму("Обработка.МаппингДанных.Форма.ТаблицаЗначений",ПараметрАдрес,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияТаблицыЗначений(РезультатЗакрытия,ДополнительныеПараметры) Экспорт 
	
	  Если РезультатЗакрытия = Неопределено Тогда		  
		  Возврат; 		  
	  КонецЕсли; 
	 	  
	  преобразоватьТаблицуЗначений(РезультатЗакрытия.АдресХранилища);  
...
&НаСервере
Процедура преобразоватьТаблицуЗначений(АдресХранилища)
	
	Если ЭтоАдресВременногоХранилища(АдресХранилища) Тогда

		//TODO (),20.08.19 , описать сохранение промежуточной таблицы, если ее необходимо будет открыть повторно 
		Результат = ЗначениеИзСтрокиВнутр(ПолучитьИзВременногоХранилища(АдресХранилища));
